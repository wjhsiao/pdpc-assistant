...
export default function AiApp() {
  ...
  return (
    <div className="min-h-screen w-full bg-[#f1f5f9] p-4 sm:p-6 box-border font-sans">
      <div className="max-w-3xl mx-auto flex flex-col gap-4">
        <header className="bg-[#0f172a] text-white rounded-2xl px-6 py-4 flex items-center gap-3">
          <div className="bg-[#8b5cf6] w-8 h-8 rounded-md flex items-center justify-center shrink-0">
            <Sparkles className="w-5 h-5 text-white" />
          </div>
          <div>
            <h1 className="font-extrabold text-[18px] tracking-[-0.5px]">PDPC AI 問答</h1>
            <div className="text-[12px] opacity-70">依函釋原文生成回答，可能耗用 API 額度</div>
          </div>
        </header>
        ...
        <main className="flex flex-col gap-4">
          {turns.length === 0 && (
            <div className="bg-white border border-[#e2e8f0] rounded-2xl p-5 text-[13px] text-[#64748b]">
          ...
          {turns.map(turn => (
            <div key={turn.id} className="flex flex-col gap-2">
              <div className="self-end max-w-[85%] bg-[#3b82f6] text-white rounded-2xl rounded-tr-sm px-4 py-2.5 text-[14px]">
                {turn.question}
              </div>

              {turn.status === 'loading' && (
                <div className="self-start flex items-center gap-2 bg-white border border-[#e2e8f0] rounded-2xl rounded-tl-sm px-4 py-3 text-[13px] text-[#64748b]">
              ...
              {turn.status === 'no-match' && (
                <div className="self-start max-w-[90%] bg-[#f8fafc] border border-[#e2e8f0] rounded-2xl rounded-tl-sm px-4 py-3 text-[13px] text-[#64748b]">
                  查無資料：找不到與此問題足夠相關的官方函釋，請嘗試換個說法，或改用首頁的關鍵字搜尋瀏覽相近條文。
                </div>
              )}
              ...
              {turn.status === 'answered' && (
                <div className="self-start max-w-[90%] bg-white border border-[#e2e8f0] rounded-2xl rounded-tl-sm px-4 py-3.5 text-[14px] leading-[1.6] text-[#1e293b]">
        ...
        <footer className="sticky bottom-4 bg-white border border-[#e2e8f0] rounded-2xl p-3 shadow-[0_4px_6px_-1px_rgba(0,0,0,0.05)]">
          ...
          <div className="mt-2 flex items-center gap-2">
            {modelStatus === 'loading' ? (
              <>
                ...
                <span className="text-[12px] font-semibold text-[#f59e0b]">語意模型載入中...</span>
              </>
            ) : modelStatus === 'error' ? (
              <>
